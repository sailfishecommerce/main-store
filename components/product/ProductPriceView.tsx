...
export default function ProductPriceView({ product }:any) {
  return (
    <div className="border rounded-lg p-4">
      <div className="price flex items-center mb-8">
        <h4 className="font-bold">{product.price}</h4>
        {product.oldPrice && (
          <h4 className="gray-manatee">{product.oldPrice}</h4>
        )}
        <div className="buy-now flex item-center justify-between">
          <div className="button-group flex items-center">
            <button className="button mr-4 mountain-green py-2 px-4">
              Add to cart
            </button>
            <button className="button tan-hide py-2 px-4">Buy now</button>
          </div>
